...
import { deleteTastingAction } from "@/app/actions/saveTasting";

export default function TastingRowActions({ id }: { id: string }) {
  ...
  async function handleDelete() {
    setIsDeleting(true);
    try {
      const res = await deleteTastingAction(id);
      if (res.success) {
        setShowModal(false);
        router.refresh();
      } else {
        alert("Error al eliminar la cata: " + res.error);
        setIsDeleting(false);
      }
    } catch (e) {
  ...
  return (
    <>
      <div className="flex items-center justify-end space-x-3">
        {/* 
          // @ts-ignore */}
        <Link href={`/admin/tastings/${id}` as any} className="text-[var(--color-gold)] hover:text-white transition-colors" title="Editar cata">
          <Pencil size={18} />
        </Link>
        <button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 text-left">
          <div className="bg-[#141414] border border-[var(--color-charcoal)] p-6 max-w-sm w-full relative">
            ...
            <p className="text-gray-300 text-sm mb-8">
              ¿Seguro que deseas eliminar esta cata de forma permanente? Solo se puede eliminar si no tiene reservas asociadas.
            </p>

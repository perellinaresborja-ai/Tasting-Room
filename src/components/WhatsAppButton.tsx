...
export default function WhatsAppButton() {
  ...
  // Retrasamos la aparición 3 segundos para que no sea intrusivo al cargar de golpe
  useEffect(() => {
    const timer = setTimeout(() => setIsVisible(true), 3000);
    return () => clearTimeout(timer);
  }, []);

  // No mostrar en paneles de administración ni área de cliente
  if (pathname.includes('/admin') || pathname.includes('/member') || pathname.includes('/scanner')) {
    return null;
  }
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-center gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="bg-[#111] border border-[var(--color-charcoal)] px-5 py-3 rounded-full shadow-2xl hidden sm:block relative">
        ...
        <div className="absolute top-1/2 -right-[7px] transform -translate-y-1/2 border-[6px] border-transparent border-l-[var(--color-charcoal)]"></div>
      </div>
